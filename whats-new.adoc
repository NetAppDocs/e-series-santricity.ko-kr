...
[role="lead"]
다음 표에서는 SANtricity System Manager 11.8x의 새로운 기능에 대해 설명합니다.
...
새 기본 인증서 키 크기는 3072비트(2048비트)입니다. 최대 4096비트의 키 크기가 지원됩니다. 기본값이 아닌 키 크기를 지원하려면 NVSRAM 비트를 변경해야 합니다.
 ...
 a| 
512b 블록 크기의 드라이브를 사용하여 더 큰 동적 디스크 풀 크기 지원
 a| 
버전 1 풀 버전 유형을 사용하는 고객은 이제 최대 크기가 48TB인 디스크 풀을 사용할 수 있습니다. 이전 버전은 최대 24TB의 디스크 풀 크기를 지원합니다. 기존 디스크 풀을 더 큰 크기로 변환하려면 먼저 모든 볼륨을 제거해야 합니다. 버전 1 디스크 풀이 있는 경우 다운그레이드 작업이 제한됩니다.
