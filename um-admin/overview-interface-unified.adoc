...
---
= SANtricity Unified Manager에 대해 알아보세요
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
관리
 a| 
네트워크에서 스토리지 어레이를 검색하고, 어레이용 SANtricity System Manager를 실행하고, 하나의 어레이에서 여러 어레이로 설정을 가져오고, 어레이 그룹을 관리할 수 있습니다. 어레이 이름 옆의 확인란을 선택하면 설정 가져오기 및 어레이 그룹 생성과 같은 작업을 수행할 수 있습니다. 각 행 끝에 있는 줄임표를 클릭하면 어레이 이름 변경과 같은 단일 어레이에 대한 작업을 위한 인라인 메뉴가 나타납니다.
